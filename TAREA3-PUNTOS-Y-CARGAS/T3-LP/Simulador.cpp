...
Simulador::Simulador(int h, int w){
    m_inter = h;
    n_inter = w;
}
...
void Simulador::Calcular(){
    float distancia;
    for(std::vector<Puntos*>::iterator it_punt=Punt.begin() ; it_punt!=Punt.end() ; it_punt++){
        for(std::vector<Carga*>::iterator it_carg = Carg.begin() ; it_carg!=Carg.end() ; it_carg++){
...
void Simulador::SetPuntos(int h, int w){
    m_inter = h;
    n_inter = w;
}
